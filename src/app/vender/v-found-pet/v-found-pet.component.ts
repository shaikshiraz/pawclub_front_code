...
@Component({
  selector: 'app-v-found-pet',
  templateUrl: './v-found-pet.component.html',
  styleUrls: ['./v-found-pet.component.css']
})
export class VFoundPetComponent implements OnInit {
  dropdownList1 = [];
  dropdownList2 = [];
  dropdownList3 = [];
  selectedItems = [];
  dropdownSettings = {};
  FoundForm: FormGroup;
  @ViewChild('image') FoundUserImage;

  FoundImageFile: File;
  constructor(private fb: FormBuilder,private _found:VenderService) {
    ...
    public Found(value) {
    const Image = this.FoundUserImage.nativeElement;
    if (Image.files && Image.files[0]) {
      this.FoundImageFile = Image.files[0];
    }
    const ImageFile: File = this.FoundImageFile;
    ...
    fd.append('phoneno',value.phoneno);
     ...
     fd.append('foundDate',value.foundDate);
      fd.append('foundLocation',value.foundLocation);
  ...
  addField(ind) {
    const fArr = this.FoundForm.get('cat1') as FormArray;
    fArr.insert(ind + 1, this.cat1());
  }
  addField1(ind) {
    const fArr = this.FoundForm.get('Other1') as FormArray;
